...
const ActivityList: React.FC<ActivityListProps> = ({ activities, dispatch }) => {
    ...
    return (
        <>
            ...
            {isEmptyActivities ? (
                <p className="text-center my-5">No hay {selectedOption === 'comidas' ? 'comidas' : 'actividades'} aún...</p>
            ) : (
                filteredActivities.map((activity) => (
                    <div key={activity.id} className="px-5 py-10 bg-white mt-5 flex justify-between shadow border-blue-600">
                        <div className="space-y-2 relative">
                            <p
                                className={`absolute -top-8 -left-8 px-10 py-2 text-white uppercase font-bold border-blue-800 ${
                                    activity.category === 1 ? 'bg-lime-500' : 'bg-blue-500'
                                }`}
                            >
                                {activity.category === 1
                                    ? exerciseCategories.find((cat) => cat.id === activity.category)?.name
                                    : foodCategories.find((cat) => cat.id === activity.category)?.name}
                            </p>
